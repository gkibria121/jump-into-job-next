...
const FeaturedJobs = () => {
  ...
  const isMobileScreen = useMediaQuery({ query: "(max-width: 1368px)" });
  useEffect(() => {
    fetch("/alljobs.json")
      .then((res) => res.json())
      .then((data) => {
        setAllJobs(data);
        const filteredFeaturedJobs = allJobs.filter(
          (job) => job.job_vacancy >= 5
        );
        const sortedFeaturedJobs = filteredFeaturedJobs.sort(
          (a, b) => b.job_vacancy - a.job_vacancy
        );
    ...
    <div className="featured-jobs-section">
      <div className="featured-jobs-content container">
        <div className="featured-jobs-header section-header">
          ...
          <p>
            Featured jobs are shown based on the number of the vacancies
            available in particuler job. So the users may know where they should
            apply or not.
          </p>
        </div>
        <div className="featured-jobs-items all-jobs-container">
          {featuredJobs.slice(0, 6).map((job) => (
              ...
              <div className="single-job-bottom">
                <p className="single-job-salary">
                  {job.salary} <span>(Estimated)</span>
                </p>
                <p className="posting-date">22d</p>
              </div>
              {isMobileScreen ? (
                <Link
                  onClick={() => handleClickedFeaturedJob(job.id)}
                  href={`/jobdetailsres/${job.id}`}
                >
                  View Details
                </Link>
              ) : (
                <Link
                  onClick={() => handleClickedFeaturedJob(job.id)}
                  href={`/findjobs/jobdetails/${job.id}`}
                >
                  View Details
                </Link>
              )}
